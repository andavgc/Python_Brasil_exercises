...
while continuar:
    ...
    if prestacao == 0:
        continuar = False
        break
    atraso = int(input('Insira os dias em atraso: '))
    pag = functions.ValorPagamento(prestacao, atraso)
    pagamentos.append(pag)
    print(f'O valor a ser pago é R${pag:.2f}')

qtd = len(pagamentos)
total = sum(pagamentos)

print(f'Foram realizadas {qtd} prestações, dando um valor total de R${total}')